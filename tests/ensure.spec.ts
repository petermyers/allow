...
describe('ability', () => {
    it('allows a user to perform an action', () => {
        ...
        expect(
            userAbility.ensure(user).canPerform(Permissions.READ).on(entity)
        ).toBeTruthy()
        expect(() =>
            userAbility.ensure(user2).canPerform(Permissions.READ).on(entity)
        ).toThrow(NotAuthorizedError)
        expect(() =>
            userAbility.ensure(user).canPerform(Permissions.UPDATE).on(entity)
        ).toThrow(NotAuthorizedError)
    })

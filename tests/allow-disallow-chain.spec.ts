...
describe('ability', () => {
    it('evaluates an allow disallow chain', () => {
        ...
        expect(
            userAbility.permits(user1).toPerform(Permissions.READ).on(entity)
        ).toBeTruthy()
        expect(
            userAbility.permits(user2).toPerform(Permissions.READ).on(entity)
        ).toBeTruthy()
        expect(
            userAbility.permits(user3).toPerform(Permissions.READ).on(entity)
        ).toBeFalsy()
        expect(
            userAbility.permits(user4).toPerform(Permissions.READ).on(entity)
        ).toBeTruthy()
        expect(
            userAbility.permits(user5).toPerform(Permissions.READ).on(entity)
        ).toBeFalsy()
    })
})

...
describe('ability', () => {
    it('allows a user to perform an action', () => {
        const userAbility = new Ability<User>((abilityInterface) => {
            ...
            allow(
                Permissions.READ,
                'type.custom',
                (user: User, entity: { userId: string }) => {
                    return (
                        entity.userId === user.id && user.roles.includes('role')
                    )
                }
            )
        })
        ...
        expect(
            userAbility
                .permits(user)
                .toPerform(Permissions.READ)
                .on(entity, 'type.custom')
        ).toBeTruthy()
        expect(
            userAbility
                .permits(user2)
    ...
    it('disallows a user from performing an action', () => {
        const userAbility = new Ability<User>((abilityInterface) => {
            ...
            allow(Permissions.READ, 'type.custom', (user: User) => {
                return user.roles.includes('role')
            })

            disallow(Permissions.READ, 'type.custom', (user: User) => {
